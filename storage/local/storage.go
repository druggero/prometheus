...
type MemorySeriesStorageOptions struct {
	TargetHeapSize             int           // Desired maximum heap size.
	...
	s := &MemorySeriesStorage{
		...
		memChunks: prometheus.NewGaugeFunc(
			prometheus.GaugeOpts{
				...
				Subsystem: subsystem,
				Name:      "memory_chunks",
				Help:      "The current number of chunks in memory, excluding cloned chunks (i.e. chunks without a descriptor).",
			},
...
func (s *MemorySeriesStorage) Start() (err error) {
	...
	s.series.Set(float64(s.fpToSeries.length()))
